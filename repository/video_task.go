...
func ListUserVideoTasks(userID string, source string, limit int) ([]model.VideoTask, error) {
	...
	err = query.
		Where("status IN ?", []string{"queued", "in_progress", "processing", "running", "not_start", "not_started", "submitted", "pending", "in_queue", "dispatched", "accepted", "started", "generating"}).
...
func ListDueVideoTasks(limit int) ([]model.VideoTask, error) {
	...
	var tasks []model.VideoTask
	err = db.Where("status IN ?", []string{"queued", "in_progress", "processing", "running", "not_start", "not_started", "submitted", "pending", "in_queue", "dispatched", "accepted", "started", "generating"}).
