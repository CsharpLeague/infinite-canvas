...
export function CanvasAssistantPanel({ nodes, selectedNodeIds, sessions, activeSessionId, onSelectNodeIds, onSessionsChange, onInsertImage, onInsertText, onPasteImage, onCollapseStart, onCollapse }: CanvasAssistantPanelProps) {
    ...
    const sendMessage = async (text: string, nextMode: AssistantMode, history: CanvasAssistantMessage[], savedReferences?: CanvasAssistantReference[]) => {
        const requestConfig =
            nextMode === "image"
                ? { ...effectiveConfig, count: effectiveConfig.canvasImageCount || effectiveConfig.count, model: effectiveConfig.imageModel || effectiveConfig.model, activeChannelId: effectiveConfig.imageChannelId, imageChannelId: effectiveConfig.imageChannelId }
                : { ...effectiveConfig, count: effectiveConfig.count, model: effectiveConfig.textModel || effectiveConfig.model, activeChannelId: effectiveConfig.textChannelId, textChannelId: effectiveConfig.textChannelId };
            ...
            <div className="rounded-[28px] border px-3 pb-3 pt-3 shadow-lg" style={{ background: theme.toolbar.panel, borderColor: theme.node.stroke }}>
                ...
                <div className="mt-2 flex items-center justify-between gap-2">
                    <div className="canvas-composer-tools flex min-w-0 flex-1 items-center gap-1">
                        ...
                        {mode === "image" ? (
                            <>
                                <ModelPicker className="h-8 shrink-0" config={config} value={config.imageModel || config.model} channelId={config.imageChannelId} onChange={(model, channelId) => { onConfigChange("imageModel", model); if (channelId) onConfigChange("imageChannelId", channelId); }} capability="image" onMissingConfig={onMissingConfig} />
                            ...
                            <ModelPicker className="h-8 shrink-0" config={config} value={config.textModel || config.model} channelId={config.textChannelId} onChange={(model, channelId) => { onConfigChange("textModel", model); if (channelId) onConfigChange("textChannelId", channelId); }} capability="text" onMissingConfig={onMissingConfig} />
                        )}
                    </div>
                    <Button
                        type="primary"
